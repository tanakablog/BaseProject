...
namespace MB
{
	...
	public abstract class ScrollCtrlBase<T> : MonoBehaviour where T : ScrollBoardCtrlBase
	{
		...
		public virtual void Init (int num)
		{
			...
			// 最大インデックス取得 ※indexは0開始なので補正
			m_MaxIndex = num - 1;

			// 最大開始ライン数算出
			m_MaxStartLine = IndexToLine (num) - m_HeightBoardNum + add_num;
		...
		public void SetStartLine (int start)
		{
			...
			int start_index = start * m_WidthBoardNum; // 横の個数を掛けている

			// オーバー数算出
			int end_index = start_index + m_BoardCtrls.Count - 1;
			int over = end_index - m_MaxIndex;
			...
			{
				UpdateBoard (m_BoardCtrls [i - start_index], i);
			}
